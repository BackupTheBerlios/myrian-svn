...
class Optimizer extends Actor {
    ...
    public final static String versionId = "$Id: //core-platform/proto/src/com/arsdigita/persistence/proto/engine/rdbms/Optimizer.java#2 $ by $Author: rhs $, $DateTime: 2003/02/05 18:34:37 $";
    ...
    public void act(Table table) {
        ...
        if (table.getSelections().size() == 0) {
            if (table.isEliminable()) {
                ...
                OldCondition one = (OldCondition) it.next();
                OldCondition two = (OldCondition) it.next();
                OldCondition added = null;
                if (one.getHead().getTable().equals(table) &&
                    ...
                    if (!one.isOuter() && !two.isOuter()) {
                        added = new OldCondition(table.getNode(),
                                                 one.getTail(),
                                                 two.getTail());
                    }
                    ...
                    added = new OldCondition(table.getNode(), one.getTail(),
                                             two.getHead());
                } else if (one.getTail().equals(two.getHead())) {
                    added = new OldCondition(table.getNode(), two.getTail(),
                                             one.getHead());
                }
