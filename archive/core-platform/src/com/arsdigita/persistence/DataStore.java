...
public class DataStore {
    ...
    public static final String versionId = "$Id: //core-platform/dev/src/com/arsdigita/persistence/DataStore.java#1 $ by $Author: dennis $, $DateTime: 2002/05/12 18:23:13 $";
    ...
    public ResultSet fireOperation(Operation op, DataContainer source,
        ...
        if (lazyUpdates && el.isUpdate()) {
            log.debug("Before processing update: " + el);
            el = processUpdate((Statement) el, source);
            log.debug("After processing update: " + el);
            if (el == null) {
